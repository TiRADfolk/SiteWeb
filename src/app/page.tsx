...
export default async function HomePage() {
  ...
  const heroBg =
    formatDriveImageUrl(config.lienBandeau) ||
    uiText.common.fallbackImage;
  ...
  return (
    <div className="space-y-12">
      ...
      >
        <div className="p-6 max-w-3xl space-y-4">
          <h1 className="text-4xl md:text-6xl font-extrabold tracking-tight">
            {config.nom || uiText.home.heroTitle}
          </h1>

          <p className="text-lg md:text-xl text-gray-200">
            {config.slogan || uiText.home.heroSubtitle}
          </p>
        </div>
      </section>
      ...
      <section className="bg-white p-6 rounded-xl shadow-sm border border-gray-100 flex flex-col md:flex-row items-center gap-6 max-w-4xl mx-auto">
        ...
        <div className="space-y-3 text-center md:text-left flex-grow">
          <h2 className="text-2xl font-bold text-[#2C221E]">
            {config.presentationTitre}
          </h2>

          <p className="text-gray-700 whitespace-pre-line leading-relaxed">
            {config.presentationTexte}
          </p>

          <Link
            href="/presentation"
            className="inline-flex items-center justify-center gap-2 bg-[#A0522D] hover:bg-[#804020] text-white font-bold py-2 px-4 rounded-lg transition shadow"
          >
            <SearchIcon />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          {upcomingEvents.length === 0 ? (
            ...
            <div className="space-y-3">
              {upcomingEvents.map((event) => (
                <div
                  key={event.id}
                  className="p-3 bg-amber-50/50 rounded-lg border border-amber-100 flex justify-between items-center"
                >
                  <div>
                    ...
                    <p className="text-xs text-gray-600">
                      {event.estPublic?.toLowerCase() === 'oui'
                        ? uiText.agenda.public
                        : uiText.agenda.private}
                    </p>
                  </div>

                  <Link
                    href="/agenda"
                    className="inline-flex items-center justify-center gap-1 bg-[#A0522D] text-white p-2 rounded hover:bg-[#804020] transition"
                  >
                    <SearchIcon />
                  </Link>
                </div>
        ...
        <section className="bg-white p-6 rounded-xl shadow-sm border border-gray-100 space-y-4">
          <h2 className="text-2xl font-bold text-[#2C221E] border-b-2 border-[#A0522D] pb-2">
            {uiText.nav.contact}
          </h2>

          <p className="text-gray-700 leading-relaxed text-sm">
            Pour toute demande de réservation, concert, bal folk ou
            renseignement, n'hésitez pas à nous contacter directement.
          </p>
          ...
          <div className="space-y-4 text-gray-800 pt-2">
            ...
            {config.adresse && (
              <div className="p-4 bg-amber-50/50 rounded-lg space-y-1">
                <span className="font-bold text-[#2C221E] block">
                  Adresse & Siège :
                </span>
